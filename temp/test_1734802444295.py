...
if __name__ == "__main__":
    # Replace inputs with hardcoded values for additional test cases
    # Test Case 1
    inorder = [4, 2, 5, 1, 6, 3]  
    ...
    print(f"Spiral Order: {spiral_order}")  # Expected: [[1], [3, 2], [4, 5, 6]]
    ...
    print(f"Lowest Common Ancestor of {p} and {q}: {lca.val if lca else None}")  # Expected: 2
    
    # Root to Leaf Paths
    paths = print_all_paths(root)
    print(f"Root to Leaf Paths: {paths}")  # Expected: [[1, 2, 4], [1, 2, 5], [1, 3, 6]]

    # Test Case 2 - Another tree
    inorder = [10, 5, 1, 7, 40, 50]
    preorder = [40, 5, 1, 10, 7, 50]
    
    root = construct_binary_tree(preorder[:], inorder[:])

    height, diameter = find_height_and_diameter(root)
    print(f"Height: {height}")  # Expected: 4
    print(f"Diameter: {diameter}")  # Expected: 5
    
    spiral_order = level_order_spiral(root)
    print(f"Spiral Order: {spiral_order}")  # Expected: [[40], [5, 50], [1, 10, 7]]
    
    balanced = is_balanced(root)
    print(f"Is Balanced: {balanced}")  # Expected: False
    
    p, q = 10, 7
    lca = find_lca(root, p, q)
    print(f"Lowest Common Ancestor of {p} and {q}: {lca.val if lca else None}")  # Expected: 5
    
    paths = print_all_paths(root)
    print(f"Root to Leaf Paths: {paths}")  # Expected: [[40, 5, 1, 10], [40, 5, 7], [40, 50]]