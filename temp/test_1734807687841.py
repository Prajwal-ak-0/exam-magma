...
if __name__ == "__main__":
    # Replace inputs with test values
    inorder = [4, 2, 5, 1, 6, 3]  # Example Test case: Inorder traversal
    preorder = [1, 2, 4, 5, 3, 6]  # Example Test case: Preorder traversal
    ...
    print(f"Height: {height}")  # Expected output: Height: 3
    print(f"Diameter: {diameter}")  # Expected output: Diameter: 4

    # Spiral Order Traversal
    spiral_order = level_order_spiral(root)
    print(f"Spiral Order: {spiral_order}")  # Expected output: Spiral Order: [[1], [3, 2], [4, 5, 6]]

    # Balanced Check
    balanced = is_balanced(root)
    print(f"Is Balanced: {balanced}")  # Expected output: Is Balanced: True

    # Lowest Common Ancestor
    p, q = 4, 5
    lca = find_lca(root, p, q)
    print(f"Lowest Common Ancestor of {p} and {q}: {lca.val if lca else None}")  # Expected output: Lowest Common Ancestor of 4 and 5: 2

    # Root to Leaf Paths
    paths = print_all_paths(root)
    print(f"Root to Leaf Paths: {paths}")  # Expected output: Root to Leaf Paths: [[1, 2, 4], [1, 2, 5], [1, 3, 6]]