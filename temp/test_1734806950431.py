...
if __name__ == "__main__":
    ...
    # Test Cases
    assert find_height_and_diameter(root) == (3, 4), "Height and Diameter test failed"
    assert level_order_spiral(root) == [[1], [2, 3], [6, 5, 4]], "Spiral Order Traversal test failed"
    assert is_balanced(root) == True, "Balanced Check test failed"
    assert find_lca(root, 4, 5).val == 2, "Lowest Common Ancestor test failed"
    assert print_all_paths(root) == [[1, 2, 4], [1, 2, 5], [1, 3, 6]], "Root to Leaf Paths test failed"
