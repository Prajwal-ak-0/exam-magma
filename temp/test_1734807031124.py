...
if __name__ == "__main__":
    ...
    inorder = [4, 2, 5, 1, 6, 3]  # Test Case 1: Valid Tree
    preorder = [1, 2, 4, 5, 3, 6]  # Test Case 1: Valid Tree
    ...
    p, q = 4, 5  # Test Case 1: Nodes to find LCA for
    ...
    inorder = [8, 4, 10, 2, 6, 12]
    preorder = [4, 2, 8, 6, 10, 12]
    root = construct_binary_tree(preorder[:], inorder[:])
    ...
    p, q = 2, 6  # Test Case 2: Nodes to find LCA for
    lca = find_lca(root, p, q)
    print(f"Lowest Common Ancestor of {p} and {q}: {lca.val if lca else None}")
    paths = print_all_paths(root)
    print(f"Root to Leaf Paths: {paths}")